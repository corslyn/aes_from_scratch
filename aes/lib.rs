...
mod tests {
    ...
    fn test_decrypt() {
        let key: [u8; 16] = [
            0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x09, 0xcf, 0x4f, 0x3c,
            0x76, 0x2e,
        ];
        let plaintext: [u8; 16] = [
        ...
        let encrypted: [u8; 16] = encrypt(plaintext, key);
    ...
    fn test_shift_rows_inverse() {
        ...
        let unshifted = shift_rows_inverse(shifted);
        assert_eq!(unshifted, state);
    }
    ...
    fn test_sub_bytes_inverse() {
        ...
        let substituted = sub_bytes(state);
        let unsubstituted = sub_bytes_inverse(substituted);
        assert_eq!(unsubstituted, state);
    }

    #[test]
    fn test_mix_columns_inverse() {
        let state: [u8; 16] = [
            0xdb, 0x13, 0x53, 0x45, 0xf2, 0x0a, 0x22, 0x5c, 0x01, 0x01, 0x01, 0x01, 0xc6, 0xc6,
            0xc6, 0xc6,
        ];
        let mixed = mix_columns(state);
        let unmixed = mix_columns_inverse(mixed);
        assert_eq!(unmixed, state);
    }
}
